...
const MyCart = () => {
  ...
  const handleDelete=(_id)=>{
        fetch(`https://1001-brand-server.vercel.app/cart/${_id}`,{
            method:'DELETE'
        })
        .then(res=>res.json())
        .then(data=>{
        ...
        {items.map((item) => ( <div key={item._id}>
            <div className="card w-96 bg-base-100 shadow-xl image-full">
              ...
              <div className="card-body">
                ...
                <p className="text-xl">{item.brand_name}</p>
                <p className="text-xl">{item.type}</p>
                <div className="card-actions justify-end ">
                <button onClick={()=>handleDelete(item._id)} className="btn text-[#52BA5D] hover:bg-[#52BA5D] hover:text-white" >Remove</button>
             </div>
              </div>
